...
def from_new_set_target_toolchain(program_root, mbed_os_path=None):
    program = MbedProgram.from_new(program_root, mbed_os_path)
...
class TestInitialiseProgram:
    def test_from_new_local_dir_raises_if_path_is_existing_program(self, tmp_path):
        ...
        with pytest.raises(ExistingProgram):
            MbedProgram.from_new(program_root, None)
    ...
    def test_from_new_existing_mbed_os_used(self, tmp_path):
        fs_root = pathlib.Path(tmp_path, "foo")
        fs_root.mkdir()
        program_root = fs_root / "programfoo"
        program_root.mkdir()
        mbed_os_path = fs_root / "mbed-os"
        make_mbed_os_files(mbed_os_path)

        program = from_new_set_target_toolchain(program_root, mbed_os_path)

        assert program.mbed_os.root == mbed_os_path

    def test_from_new_existing_mbed_os_does_not_exist(self, tmp_path):
        fs_root = pathlib.Path(tmp_path, "foo")
        fs_root.mkdir()
        program_root = fs_root / "programfoo"
        program_root.mkdir()
        ...
        with pytest.raises(ValueError):
            from_new_set_target_toolchain(program_root, mbed_os_path)
    ...
    def test_from_existing_with_mbed_os_path_returns_valid_program(self, tmp_path):
        ...
        assert program.files.app_config_file.exists()
        assert program.mbed_os.root.exists()
        assert program.mbed_os.cmake_template_file.exists()

    def test_from_existing_mbed_os_path_no_template(self, tmp_path):
        fs_root = pathlib.Path(tmp_path, "foo")
        mbed_os_path = fs_root / "extern/mbed-os"
        mbed_os_path.mkdir(parents=True)
        make_mbed_program_files(fs_root)
        make_mbed_os_files(mbed_os_path, include_template=False)

        program = MbedProgram.from_existing(fs_root, DEFAULT_BUILD_SUBDIR, mbed_os_path)

        assert program.files.app_config_file.exists()
        assert program.mbed_os.root.exists()
        assert program.mbed_os.cmake_template_file is None
